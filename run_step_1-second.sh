...
INPUT_DIR=/Prove/Yoshihisa/HEIF_ghost/HEIF_IMAGES_LIBHEIF/HEIF_images_first
OUTPUT_DIR=/Prove/Yoshihisa/HEIF_ghost/HEIF_IMAGES_LIBHEIF/HEIF_images_second

source "/data/lesc/staff/yoshihisa/anaconda3/etc/profile.d/conda.sh"

./1-perform_encoding.py -l "$LIBHEIF_PATH" -c "$CONVERT_PATH" -i "$INPUT_DIR" -o "$OUTPUT_DIR" run-second ${SLURM_ARRAY_TASK_ID}
